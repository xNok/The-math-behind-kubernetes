...
var x{t in TIME_INTERVALS, a in APPLICATIONS, s in 1..replicas[a,t], n in NODE_TYPES, i in 1..max_nodes[n]} binary; # 1 if replica 's' of 'a' is on node 'n', 0 otherwise
var y{t in TIME_INTERVALS, n in NODE_TYPES, i in 1..max_nodes[n]} binary; # 1 if node 'i' if type 'n' is used;

# Objective Function

minimize total_cost: sum{t in TIME_INTERVALS, n in NODE_TYPES, i in 1..max_nodes[n]} cost[n] * y[t,n,i];

# Constraints

## 1. Application Assignment: Each application must be assigned to a node

s.t. ApplicationAssignment{t in TIME_INTERVALS, a in APPLICATIONS, s in 1..replicas[a,t]}:
  sum{n in NODE_TYPES, i in 1..max_nodes[n]} x[t,a,s,n,i] = 1;

## 2. Resource Capacity: Total demand cannot exceed node capacity

s.t. ResourceCapacity{t in TIME_INTERVALS, n in NODE_TYPES, i in 1..max_nodes[n], resource in RESOURCES}:
  sum{a in APPLICATIONS, s in 1..replicas[a,t]} r[a,resource] * x[t,a,s,n,i] <= c[n,resource] * y[t,n,i];

## 3. Node Count: Ensure at least one node if any replica is assigned

s.t. NodeCount{t in TIME_INTERVALS, a in APPLICATIONS, s in 1..replicas[a,t], n in NODE_TYPES, i in 1..max_nodes[n]}:
  x[t,a,s,n,i] <= y[t,n,i];

## 4. Replica Anti-Affinity: Replicas of the same application on different nodes

s.t. ReplicaAntiAffinity{t in TIME_INTERVALS, a in APPLICATIONS, s1 in 1..replicas[a,t], s2 in 1..replicas[a,t], n in NODE_TYPES, i in 1..max_nodes[n]: s1 <> s2}:
...
s.t. AntiDescheduling{t1 in TIME_INTERVALS, t2 in TIME_INTERVALS, a in APPLICATIONS, s in 1..min(replicas[a,t1], replicas[a,t2]), n in NODE_TYPES, i in 1..max_nodes[n]: t1 - t2 = 1}:
...
for {t in TIME_INTERVALS} {
    ...
    for {n in NODE_TYPES: node_count[t,n] > 0} {
      ...
      for {i in 1..max_nodes[n]: y[t,n,i] > 0} {
        printf "%d: ", i;
        for {a in APPLICATIONS, s in 1..replicas[a,t]: x[t,a,s,n,i] = 1} {
          printf "%s ", a;
        }
        printf "\n";
      }
